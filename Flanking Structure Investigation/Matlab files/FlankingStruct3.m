...
% initial state of the ellipsoid                                    
   a= [1000;.5;.1];                                                     % initial length of the ellipsoid axis
   pp= pi/2;
...
% 3D meshgrid in external coordinate
        xgv     = -2:.05:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv     =.2*ones(size(xgv));           % grid vector: y'axis,a2
        ...
        xgv_1     = -2:.05:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ...
        xgv_2     = -2:.05:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
...
%}
   ...
   %------------------------------------------------------ 
    ...
    % [~,N]=size(ep);
    parfor ir=1:N
        vel_f(:,:,ir)= Velocity_field_calc3(xxx(ir),yyy(ir),a,invS,Jd,d,u2,w,ll);
  ...
  %------------------------------------------------------ 
   ...
   vel_f1= zeros(2,1,N1);
    ...
    yyy1(isnan(yyy1)==1)=[];
     [~,N1]=size(ep_1);
    parfor ir=1:N1
        vel_f1(:,:,ir)= Velocity_field_calc3(xxx1(ir),yyy1(ir),a,invS,Jd,d,u2,w,ll);
   ...
   %------------------------------------------------------ 
    ...
    yyy2(isnan(yyy2)==1)=[];
     [~,N2]=size(ep_2);
    parfor ir=1:N2
        vel_f2(:,:,ir)= Velocity_field_calc3(xxx2(ir),yyy2(ir),a,invS,Jd,d,u2,w,ll);
    ...
    %----------------------------
     ...
     %}
        ...
        ygv1    = -2:.01:2;           % grid vector: y'axis,a2
        zgv1     = -2:.01:2;  % grid vector: z'axis,a3
       ...
       YYY= epp1(2,:);  % inclusion
        ...
        plot(XX,YY,'.','MarkerSize',3)     % this is for the marker 1 !
        hold on
        plot(XX1,YY1,'.','MarkerSize',3)     % this is for the marker 2 !
        hold on
        plot(XX2,YY2,'.','MarkerSize',3)     % this is for the marker 3 !
        hold on
        %}
        plot(XXX,YYY,'.','MarkerSize',3)    % this is for the inclusion !
        xlim([-2 2])
        ylim([-2 2])
        pbaspect([1 1 1])
        F(i)= getframe(f);
...
v= VideoWriter('FlankStruct_r=100_ia90_ar.5-.1_markers_.2-0-.2_Wk=1.avi');
v.FrameRate =2;
open(v)
writeVideo(v,F)
close(v)
