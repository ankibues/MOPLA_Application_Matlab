% Deformation of marker with exterior points around inclusion- 

% Putting circles around the inclusion to see how the strain is attained.
...
KKK1 = circlemarkerpoints();

        ygv     = KKK1(1,:);%0 + .1*cos(eta);     %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        zgv     = KKK1(2,:);%.25 + .1*sin(eta);           % grid vector: y'axis,a2
        xgv     = zeros(size(ygv)); %-5:.01:5;  % grid vector: z'axis,a3   ----> considering this 0 for now, since only consider 2D here !
     ...
     %{  
        ...
        xgv_1     =  0 + .1*cos(eta);          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_1     =  -0.25  + .1*sin(eta);          % grid vector: y'axis,a2
        ...
        xgv_2     = 0 + .1*cos(eta);         %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_2     = -0.5 + .1*sin(eta);           % grid vector: y'axis,a2
  ...
  %------------------------------------------------------ 
   ...
   vel_f1= zeros(2,1,N1);
    ...
    yyy1(isnan(yyy1)==1)=[];
     [~,N1]=size(ep_1);
    parfor ir=1:N1
        vel_f1(:,:,ir)= Velocity_field_calc5(xxx1(ir),yyy1(ir),a,invS,Jd,d,u2,w,ll);
   ...
   %------------------------------------------------------ 
    ...
    yyy2(isnan(yyy2)==1)=[];
     [~,N2]=size(ep_2);
    parfor ir=1:N2
        vel_f2(:,:,ir)= Velocity_field_calc5(xxx2(ir),yyy2(ir),a,invS,Jd,d,u2,w,ll);
    ...
    %----------------------------
     ...
     %}
        ...
        ygv1    = -3:.01:3;           % grid vector: y'axis,a2
        zgv1     = -3:.01:3;  % grid vector: z'axis,a3
       ...
       YYY= epp1(2,:);  % inclusion
        ...
        plot(XX,YY,'.','MarkerSize',.1)     % this is for the marker 1 !
        hold on
        %plot(XX1,YY1,'.','MarkerSize',.1)     % this is for the marker 2 !
        %hold on
        %plot(XX2,YY2,'.','MarkerSize',.1)     % this is for the marker 3 !
        hold on
        %}
        plot(XXX,YYY,'.','MarkerSize',.1)    % this is for the inclusion !
        xlim([-3 3])
        ylim([-3 3])
        pbaspect([1 1 1])
        F(i)= getframe(f);
...
v= VideoWriter('1FlankStruct_r=10_ia90_ar.circle_markers8_.4-0.4_Wk=0.avi');
v.FrameRate =2;
open(v)
writeVideo(v,F)
close(v)
