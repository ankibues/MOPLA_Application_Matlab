% Testing reverse dynamic modelling
% For first 25 steps, deforming forward, and then after 25 steps reversing
% the flow field

% Modelling a flanking structure
...
% 3D meshgrid in external coordinate
        xgv     = -1.5:.05:1.5;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv     = zeros(size(xgv));           % grid vector: y'axis,a2
...
for i=1:steps
    if i > 124
        ...
        [~,NN]= size(epp);
        epp1= zeros(size(epp));
        parfor rrr=1:NN                                % bringing to general coordinate system
       ...
       YYY= epp1(2,:);  % inclusion
        ...
        plot(XX,YY,'.','MarkerSize',4)     % this is for the marker 1 !
        hold on
