...
Wk=linspace(0,1,20);
LL  = reverseLgenerator(Wk);  % for N  cases of  initial L (obtained  from Wk space)
...
% initial state of the ellipsoid                                    
   ...
   %pp= 3*pi/4;          % chosen from the approximation of the CE.
...
% 3D meshgrid in external coordinate
        xgv     = XX;          %this is where  digitized X  coordinates are used
        ygv     = YY;           % this is where  digitized X  coordinates are used
 ...
 for ii=1:length(LL)
     for iii=1:length(R)
         ...
         for i=1:steps
            ...
            p_in = R_Multiply(p,h);
            E= Ed(Nm,Nc,R(iii),S,d,epsilonII,b,Jd);
            if any(isnan(E(:)))==1
                if  i==1
                ...
                num=num+1;
                Residual(num).WK= Wk(ii);
                Residual(num).viscR= R(iii);
                Residual(num).timestep= i;
                Residual(num).value= Ri/Ro;
                break
            end
            r_new= E(1,4);
