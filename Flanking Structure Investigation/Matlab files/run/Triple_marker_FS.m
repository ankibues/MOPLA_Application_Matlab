%function Triple_marker_FS(Wk,r,a,pp)% Deformation of marker with exterior points around inclusion- modelling a
% flanking structure
...
% 3D meshgrid in external coordinate
        ...
        xgv_1     = -1.5:.1:1.5;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_1     = .25*ones(size(xgv));           % grid vector: y'axis,a2
        ...
        xgv_2     = -1.5:.1:1.5;         %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_2     = -.25*ones(size(xgv));           % grid vector: y'axis,a2
   ...
   %}
    ...
    %----------------------------
     ...
     %------------------------------------------------------------
        ...
        ygv1    = -1.5:.05:1.5;           % grid vector: y'axis,a2
        zgv1     = -1.5:.05:1.5;  % grid vector: z'axis,a3
       ...
       YYY= epp1(2,:);  % inclusion
        ...
        plot(XX,YY,'k.','MarkerSize',3)     % this is for the marker 1 !
        hold on
        plot(XX1,YY1,'k.','MarkerSize',3)     % this is for the marker 2 !
        hold on
        plot(XX2,YY2,'k.','MarkerSize',3)     % this is for the marker 3 !
        hold on
       % plot(XX3,YY3,'.','MarkerSize',2)     % this is for the marker 3 !
       % hold on
       % plot(XX4,YY4,'.','MarkerSize',2)     % this is for the marker 3 !
        %
        plot(XXX,YYY,'r.','MarkerSize',3)    % this is for the inclusion !
        xlim([-1.5 1.5])
        ylim([-1.5 1.5])
        pbaspect([1 1 1])
        F(i)= getframe(f);
        ...
        if rho>2.4 && rho<2.5
            filename1=sprintf('Triple_marker_FS_r=%d_ia%d_AR=%1.1f-%1.1f-%1.1f_Wk=%1.1f_rho=%1.1f.jpg',r,rad2deg(pp),A(1),A(2),A(3),Wk,rho);
            saveas(f,filename1);
        end
        if rho>5
            filename1=sprintf('Triple_marker_FS_r=%d_ia%d_AR=%1.1f-%1.1f-%1.1f_Wk=%1.1f_rho=%1.1f.jpg',r,rad2deg(pp),A(1),A(2),A(3),Wk,rho);
...
filename=sprintf('Triple_marker_FS_r=%d_ia%d_AR=%1.1f-%1.1f-%1.1f_Wk=%1.1f_rho=%1.1f.avi',r,rad2deg(pp),A(1),A(2),A(3),Wk,rho);
