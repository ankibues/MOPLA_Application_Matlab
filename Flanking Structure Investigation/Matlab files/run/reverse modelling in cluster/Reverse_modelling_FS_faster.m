...
load('MarkerPos_nature1_10p.mat','XX','YY');
...
% initial state of the ellipsoid                                    
   ...
   A=[1000;1;.1];% initial length of the ellipsoid axis
   pp= 48*pi/180 ;          % chosen from the approximation of the CE.
  ...
  %
    parfor iiii=1:length(ic)
         ...
         for i=1:steps
            ...
            if any(isnan(E(:)))==1
                if  i==1
                    Ro= sum(sqrt(YY-mean(YY)))/length(YY);
                end
                Ri=  sum(sqrt(YY-mean(YY)))/length(YY);
