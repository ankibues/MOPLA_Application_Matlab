% Algorithm for reverse dynamic modelling

%based on range of Wk from 0-1, we generate L
% reverse Matrix flow field
k=pwd;
...
load('MarkerPos_nature2_10p.mat','XX','YY');
...
% we  take different values  of r  (CE's  viscosity  ratio to the medium)
R= logspace(-2,2);   % r is varying  between 10^-2 to 10^2
%  ------------------------------------------------------------------------   
...
% initial state of the ellipsoid                                    
   %a= [1000;.5;.1];    % chosen from the approximation of the CE.
   
   A=[1000;.15;.1];% initial length of the ellipsoid axis
   pp= 150*pi/180 ;          % chosen from the approximation of the CE.
   ang = [0;0;pp] ;                                          % initial angles of the ellipsoid axis
   q = Q(ang);          % chosen from the approximation of the CE.
   QQ=q;
...
% 3D meshgrid in external coordinate
        xgv     = XXX;          %this is where  digitized X  coordinates are used
        ygv     = YYY;           % this is where  digitized X  coordinates are used
        zgv     = zeros(size(xgv));       % considering this 0 since only consider 2D here !
        ep    = cat(1,xgv,ygv,zgv);
        [~,N]= size(ep);
        
       %{
        xgv_1     = -1.5:.05:1.5;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_1     = .3*ones(size(xgv));           % grid vector: y'axis,a2
        ...
        xgv_2     = -1.5:.05:1.5;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_2     = -.3*ones(size(xgv));           % grid vector: y'axis,a2
...
for ii=1:length(Wk)
  ...
  %Residual(length(Wk)*length(R)*steps)=  struct('WK',[],'viscR',[],'timestep',[],'value',[]);
  %
    parfor iiii=1:length(ic)
         Wk= ic(iiii).WK;
         r=  ic(iiii).viscR;
         L = reverseLgenerator2(Wk);
         D = 0.5 * (L + L');
         ...
         for i=1:steps
            ...
            if any(isnan(E(:)))==1
                if  i==1
                    Ro= sum(sqrt((YY-mean(YY)).^2))/length(YY);
                end
                Ri=  sum(sqrt((YY-mean(YY)).^2))/length(YY);
                ic(iiii).residual(i)= Ri/Ro;
                break
            end
            r_new= E(1,4);
          ...
          %{
           ...
           %}
            ...
            %
            for ir=1:N
                vel_f(:,:,ir)= Velocity_field_calc5(xxx(ir),yyy(ir),a,invS,Jd,d,u2,w,ll);
            end
            ...
            ygv1    = -1.5:.02:1.5;           % grid vector: y'axis,a2
            zgv1     = -1.5:.02:1.5;  % grid vector: z'axis,a3
      ...
      %{
            ...
            plot(XX,YY,'k.','MarkerSize',4)     % this is for the marker 1 !
            hold on
            %plot(XX1,YY1,'.','MarkerSize',4)     % this is for the marker 2 !
            %hold on
            %plot(XX2,YY2,'.','MarkerSize',4)     % this is for the marker 3 !
            %hold on
            % plot(XX3,YY3,'.','MarkerSize',2)     % this is for the marker 3 !
            % hold on
            % plot(XX4,YY4,'.','MarkerSize',2)     % this is for the marker 3 !
            %
            plot(XXX,YYY,'r.','MarkerSize',2)    % this is for the inclusion !
            xlim([-1.5 1.5])
            ylim([-1.5 1.5])
            pbaspect([1 1 1])
            F(i)= getframe(f);
        ...
        %{
         ...
         parfor rrr=1:N3                                % marker 4 bringing from general coordinate system to new rotated ellipsoid system
         newpos_plot3(:,rrr)= q*newpos_plot3(:,rrr);
         end
         parfor rrr=1:N4                                % marker 5 bringing from general coordinate system to new rotated ellipsoid system
...
%{  
K= cell2mat(struct2cell(Residual));
KK=squeeze(K);        
[minval,ind]= min(K(4,:));


   
filename=sprintf('FlankStruct_r=%d_ia%d_AR=%1.1f-%1.1f-%1.1f_Wk=%1.1f.avi',R,rad2deg(pp),A(1),A(2),A(3),Wk);
