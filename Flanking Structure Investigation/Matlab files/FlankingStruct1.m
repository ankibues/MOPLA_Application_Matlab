...
% initial state of the ellipsoid                                    
   a= [100;.8;.1];                                                     % initial length of the ellipsoid axis
   pp= 3*pi/4;
...
% 3D meshgrid in external coordinate
        xgv     = -2:.02:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv     =.4*ones(size(xgv));           % grid vector: y'axis,a2
        ...
        xgv_1     = -2:.02:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ...
        xgv_2     = -2:.02:2;          %-8:.05:8; %:.1:8;           % grid vector: x'axis,a1
        ygv_2     = -.4*ones(size(xgv));           % grid vector: y'axis,a2
    ...
    %----------------------------
     ...
     %}
       ...
       YYY= epp1(2,:);  % inclusion
        ...
        plot(XX,YY,'b.','MarkerSize',2)     % this is for the marker 1 !
        hold on
        plot(XX1,YY1,'b.','MarkerSize',2)     % this is for the marker 2 !
        hold on
        plot(XX2,YY2,'b.','MarkerSize',2)     % this is for the marker 3 !
        hold on
        %}
        plot(XXX,YYY,'b.','MarkerSize',2)    % this is for the inclusion !
        xlim([-3 3])
        ylim([-3 3])
        pbaspect([1 1 1])
        F(i)= getframe(f);
...
v= VideoWriter('FlankStruct_r=.1_ia135_ar.8-.1_markers_.4-0.4_Wk=1.avi');
v.FrameRate =2;
open(v)
writeVideo(v,F)
close(v)
