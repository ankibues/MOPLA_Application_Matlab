...
% PI  
  PI      = PI_el;
    ...
    theta= we-wE;
    sigmainside= Multiply(2*r*Jd,dE);
    ...
    % 3D meshgrid in clast's coordinate
        xgv     = 0:.1:3;           % grid vector: x'axis,a1
        ygv     = 0:.1:2;           % grid vector: y'axis,a2
  ...
  %{      
      ...
      %  p2          = p_Ex ./ Sigma_Inva;
       ...
       % pressure        = reshape(pressure,size(squeeze(Z)));
        ...
        isosurface(X,Y,Z,stress,1.2)
        isosurface(X,Y,Z,stress,.4)
       % isosurface(X,Y,Z,stress,.8)
