...
% initial state of the ellipsoid
   ...
   ang = [0; 0;(pi/180)*135];                       % pressure variations for 135 degree angle
   q = Q(ang);
...
% PI  
  PI      = PI_el;
    ...
    % 3D meshgrid in clast's coordinate
        xgv     = -10:0.1:10;           % grid vector: x'axis,a1
        ygv     = -10:0.1:10;           % grid vector: y'axis,a2
        zgv     = -10:0.1:10;    % grid vector: z'axis,a3
    ...
    pr = patch(isosurface(x,y,z,pressure,k));
    isonormals(x,y,z,f,pr)
    pr.FaceColor = 'red';
    pr.EdgeColor = 'none';
