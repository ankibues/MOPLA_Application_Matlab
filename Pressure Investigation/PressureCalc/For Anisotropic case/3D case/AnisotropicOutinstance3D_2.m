...
%--------------------------------------------------------------------------
 ...
 % 3D meshgrid in clast's coordinate
        xgv     = 0:.15:4;           % grid vector: x'axis,a1
        ygv     = 0:.15:3;           % grid vector: y'axis,a2
        zgv     = 0:.15:2;   % grid vector: z'axis,a3
...
[psi1,Win1] = Gauss1(100);

[Alpp2,Bett2,www2] = GaussGGQ(100);
...
%-----------------------------------------------------------------
        ...
        str= sprintf('pressurefield_r=%1.1f_m=%d.mat',r,m);
        save(str,pressure);
        %figure
        %isosurface(X,Y,Z,pressure,0)
        %isosurface(X,Y,Z,pressure,1)
        %isosurface(X,Y,Z,pressure,-1)
%}       