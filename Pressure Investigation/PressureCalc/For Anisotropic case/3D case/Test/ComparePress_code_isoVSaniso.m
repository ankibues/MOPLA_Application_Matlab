% Test of Exterior Pressure calculation Analytical vs Numerical 3D Anisotropic case !

L     = [1 0 0;0 -1 0;0 0 0];  % Macroscale flow field given by velocity gradient tensor
D = 0.5 * (L + L');

%  orientation and shape of inclusion. See Qu et al (2016) for more details

ang = [0; 0; 0];  % these sperical angles correspond to the case when a1 lie along Z-axis, a2 along X-axis and a3 along Y-axis 
q = Q(ang);
d= q*D*q';        
a=[3;1.5;1];      
...
Cm= 2*Jd;   % stiffness matrix for the isotropic medium
 ...
 r=.1;      % viscosity ratio of the inclusion's viscosity to the embedding medium.
 
 
 %-------------------------------------------------------------------------------
 %--------------Quasi-analytical solution for isotropicmedium------------
 %-------------------------------------------------------------------------------
...
u2= dE-d;
 ...
 % Exterior Fields  
        ...
        % p_Ex: exterior field pressure
            ...
            p_Ex    = uu3 ; %pressure at an exterior point using isotropic formulation
 ...
 % Exterior fields (p_Ex)
  ...
  LAMBDA_Ex2= zeros(3,3,1);
        ...
        % --------------------------------------------------------------
         ...
         if lambda<= .64
             LAMBDA_Ex2(:,:,1) = LambdaExtAniso3DPress_vec(a,ep(:,1),Cm,theta1,phi1,Wout1,psi1,Win1);
         elseif lambda>.64 && lambda<= 1.36
             LAMBDA_Ex2(:,:,1) = LambdaExtAniso3DPress_vec(a,ep(:,1),Cm,theta2,phi2,Wout2,psi2,Win2);
         elseif lambda> 1.36 && lambda<= 2.16
             LAMBDA_Ex2(:,:,1) = LambdaExtAniso3DPress_vec(a,ep(:,1),Cm,theta3,phi3,Wout3,psi3,Win3);
         else 
             LAMBDA_Ex2(:,:,1) = LambdaExtAniso3DPress_vec(a,ep(:,1),Cm,theta4,phi4,Wout4,psi4,Win4);
         end
         ...
         % depending on the shape of inclusion, more number of Gaussian
         % points might be needed to get accurate results.
