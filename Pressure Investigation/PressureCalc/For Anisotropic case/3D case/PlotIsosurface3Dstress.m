% Internal and External Stress field for an isotropic ellipse in planar anisotropic Material
 k=pwd;
...
%--------------------------------------------------------------------------
 ...
 % 3D meshgrid in clast's coordinate
        xgv     = 0:.2:4;           % grid vector: x'axis,a1
        ygv     = 0:.2:3;           % grid vector: y'axis,a2
        zgv     = 0:.2:2;   % grid vector: z'axis,a3
...
%-----------------------------------------------------------------
        ...
        % Lambda_External 
         if lambda(1,rr)<= .41
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta1,phi1,Wout1,psi1,Win1);
         elseif lambda(1,rr)>.41 && lambda(1,rr)<= .84
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta2,phi2,Wout2,psi2,Win2);
         elseif lambda(1,rr)> .84 && lambda(1,rr)<= 1.76
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta3,phi3,Wout3,psi3,Win3);
         elseif lambda(1,rr)> 1.76 && lambda(1,rr)<= 2.25
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta4,phi4,Wout4,psi4,Win4);
         elseif lambda(1,rr)> 2.25 && lambda(1,rr)<= 2.76
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta5,phi5,Wout5,psi5,Win5);    
         else 
             T_Ex(:,:,:,:,rr) = T_ExtAniso3D(a,ep(:,rr),Cm_a,theta6,phi6,Wout6,psi6,Win6);
         end
        ...
        str= sprintf('stressfield_pureshear_r=%1.1f_m=%d.mat',r,m);
        save(str,'stress');
        save('Stress_3D_highres_undersimple_shear.mat', 'stress')
        figure
        isosurface(X,Y,Z,stress,0)
