...
% initial state of the ellipsoid
   ...
   a= [10;6;1];                   % Point to note: as ellipsoidal long axis is reduced, results vary with respect 
...
for i=1:180
  ...
  [S_el,PI_el] = SnP(a);
LL= -1/3*(S_el(:,:,1,1) + S_el(:,:,2,2)+ S_el(:,:,3,3));
...
% PI  
  PI      = PI_el;
   %**************************
  S_el(:,:,1,1)= S_el(:,:,1,1) +LL;
   S_el(:,:,2,2)= S_el(:,:,2,2) +LL;
     S_el(:,:,3,3)= S_el(:,:,3,3) +LL;
   ...
   StressInvaRatio(i,1)= Sigma_Inva_ellipsoid/Sigma_Inva;
