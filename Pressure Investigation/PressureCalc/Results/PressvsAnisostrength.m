...
%function PressvsAnisostrength(m)
%  Input parameters--------------------------------------------------------
%  Matrix flow field
     L     = [1 0;0 -1];  
   %ep=0.5;
...
%  generate 4th-order identity tensors   
   [Jd, ~, Ja, ~] = FourIdentity2D();
   
    %sqrtm= (m)^0.5;
 %  length of the ellipse axises
   a= [5;1];                        
  % R= 5;
    Pdev_steps_alpha1= zeros(100,1);
    Pdev_steps_alpha2= zeros(100,1);
...
%  tensor W        
   ...
   % stiffness tensor of the matrix(in ellipse coordinate system),(Jiang 2016, Eq-52)
        ...
        Cm(1,1,1,1)= Nn*((cos(2*ang))^2 + ((sin(2*ang))^2)/m(i+1));
        Cm(2,2,2,2)=  Cm(1,1,1,1);
        Cm(1,1,2,2)= -Cm(1,1,1,1);
        Cm(1,1,1,2)= Nn*((1/m(i+1))-1)*sin(2*ang)*cos(2*ang);
        Cm(1,2,2,2)= -Cm(1,1,1,2);
        Cm(1,2,1,2)= Nn*((sin(2*ang))^2 + ((cos(2*ang))^2)/m(i+1));
...
% Eshelby Tensors (S_el,PI_el) for Interior points  
         [S,LAMBDA,PI] = SnpInAniso(a,Jd,Ja,ang,Nn,m(i+1),Cm,Alp,ww);
