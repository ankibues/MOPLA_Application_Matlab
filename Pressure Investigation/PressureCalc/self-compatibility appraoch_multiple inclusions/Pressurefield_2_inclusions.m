% This script calculates pressure field in and around two heterogeneities close to each other
...
% S  
  ...
  % for 1st inclusion 
    ...
    e1       = Multiply(fdE(Nm,r1,S1,b,Jd),d1);   % e1 is strain rate tensor in ellipsoid 1               
    u1= e1-d1;                          
  ...
  % for 2nd inclusion 
    ...
    e2       = Multiply(fdE(Nm,r2,S2,b,Jd),d2);   % e2 is strain rate tensor in ellipsoid 2               
    u2= e2-d2;                          
...
dE_check= zeros(3,3,1000);
  for i=1:1000
   ...
   Ex2_1= Extfield(a1,EP2,invS1,u1,d1); % at 2 because of 1 
   ...
   delta_E1 = norm(d_1-d1); 
   delta_E2 = norm(d_2-d2);
   kkk = delta_E1;%max(delta_E1,delta_E2);
