function G= Gtensor_El(a,x,mu)
% Ex_Gtensor.m
% Exterior-point Eshelby tensor (G_bar) for Elastic material based on Gao & Ma,2010
%
...
s1     = S1(lambda, a, mu);                      %s1: 3*3*N
s2     = S2(lambda, a, mu);                      %s2: 3*3*N
s3     = S3(lambda, a, q, omega, mu);            %s3: 3*N
s4     = S4(lambda, a, q, omega, mu);            %s4: 3*N 
s5     = S5(lambda, a, q, omega, mu);            %s5: 3*N 
s7     = S7(lambda, a, q, q1, omega, mu);        %s7: 3*3*3*3*N
...
for i=1:3
    for j=1:3
       s1(i,j,:) = (1/8/pi/(1-mu)).*(2.*mu.*J(i,:)-J(j,:)+a(i)^2.*(squeeze(JJ(i,j,:)))');
    end
end
end

function s2 = S2(lambda, a, mu)
%s2: 3*3*N
[~,n]  = size(lambda);
J      = J1(lambda, a);
JJ     = J2(lambda, a);
...
for i=1:3
    for j=1:3
      s2(i,j,:) = (1/8/pi/(1-mu)).*((1-mu).*J(i,:)-mu.*J(j,:)+a(i)^2.*(squeeze(JJ(i,j,:)))');
    end
end
end

function s3 = S3(lambda, a, q, omega, mu)
%s3: 3*N
u      = [a(1)^2+lambda; a(2)^2+lambda; a(3)^2+lambda];
delta  = (u(1,:).*u(2,:).*u(3,:)).^0.5;
t      = (a(1)*a(2)*a(3)).*lambda./2./(1-mu)./delta./q; %t:1*N
tt     = repmat(t,3,1); 
s3     = tt.*omega;
end

function s4 = S4(lambda, a, q, omega, mu)
%s4: 3*N 
u      = [a(1)^2+lambda; a(2)^2+lambda; a(3)^2+lambda];
delta  = (u(1,:).*u(2,:).*u(3,:)).^0.5;
t      = (a(1)*a(2)*a(3))./2./(1-mu)./delta./q;
t1     = repmat(t,3,1);
t2     = (repmat(lambda,3,1).*omega-2*mu); 
s4     = t1.*t2;
end

function s5 = S5(lambda, a, q, omega, mu)
%s5: 3*N 
u      = [a(1)^2+lambda; a(2)^2+lambda; a(3)^2+lambda];
delta  = (u(1,:).*u(2,:).*u(3,:)).^0.5;
t      = (a(1)*a(2)*a(3))./2./(1-mu)./delta./q;
t1     = repmat(t,3,1);
t2     = (repmat(lambda,3,1).*omega-1+mu);
s5     = t1.*t2;
end

function s7 = S7(lambda, a, q, q1, omega, mu)
%s7: 3*3*3*3*N
[~,n]  = size(lambda);
u      = [a(1)^2+lambda; a(2)^2+lambda; a(3)^2+lambda];
delta  = (u(1,:).*u(2,:).*u(3,:)).^0.5;
...
for i=1:3
    for j=1:3
        for k=1:3
            for l=1:3
                v(k,l,:)=(a(1)*a(2)*a(3))./delta./q.^2./2./(1-mu).*...
