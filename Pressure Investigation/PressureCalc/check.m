...
theta= [0 30 60];
for i=1:3
    Evol_Ellipsoid_Press_calc_inside(theta(i),0,1)
    Evol_Ellipsoid_Press_calc_inside(theta(i),.5,1)
...
viscosity_ratio_parameter(a1);
hold on 
viscosity_ratio_parameter(a2);
hold on
viscosity_ratio_parameter(a3);

%}
%{
Evol_Ellipsoid_Press_calc_inside(0,0,1)
Evol_Ellipsoid_Press_calc_inside(30,0,1)
Evol_Ellipsoid_Press_calc_inside(60,0,1)

Evol_Ellipsoid_Press_calc_inside(0,.5,1)
Evol_Ellipsoid_Press_calc_inside(30,.5,1)
Evol_Ellipsoid_Press_calc_inside(60,.5,1)

Evol_Ellipsoid_Press_calc_inside(90,.5,1)
Evol_Ellipsoid_Press_calc_inside(120,.5,1)
Evol_Ellipsoid_Press_calc_inside(150,.5,1)
%}
...
% initial state of the ellipsoid
   ...
   ang = [0; 0;(pi/180)*90];                       % pressure variations for 135 degree angle
   q = Q(ang);
...
% PI  
  PI      = PI_el;
    ...
    % 3D meshgrid in clast's coordinate
        xgv     = -15:1:15;           % grid vector: x'axis,a1
        ygv     = -15:1:15;           % grid vector: y'axis,a2
     ...
     %   [a1ang, a2ang, a3ang] = ConvertQ2Angs2(q);
        ...
        pressure(ind)   = p2;
     [~,~,ll] = size(X);
pressures = reshape(pressure,[ll,ll,ll]);
%}
...
for i=1:numel(r)
   ...
   PressureInsideIsotropic(r(i));
   hold on 
   PressAnisoInplot(r(i))
...
m= [1 2 10 100];
for i = 1:4
    PressFunc_m_diff_r(10,m(i))
    hold on
    PressFunc_m_diff_r(.1,m(i))
end
%}